...
def get_year_month(issuance_date):
    ...
    if format_found:
        if int(year) <= 2009:
            year = np.nan ### This is a missing value, and will be replaced later
        return '{}-{}'.format(year, month)
    else:
        print issuance_date


permits = pd.read_csv('./../data/Multi_Agency_Permits.csv',header=0,usecols = ['Borough_Block_Lot', 'Permit_Issuance_Date', 'Permit_Type_Description'], dtype={'Permit_Issuance_Date':str})
